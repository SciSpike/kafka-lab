# `wordcount-kafka-streaming` — build & run

Provided Java for the [wordcount-kafka-streaming](../wordcount-kafka-streaming.md) lab — the Kafka Streams word-count topology.
...
docker run --network docker_kafka_network --rm -it -v "$PWD:/pwd" -w /pwd openjdk:11 \
  java -jar target/wordcount-kafka-solution-1.0.0-SNAPSHOT.jar
```
