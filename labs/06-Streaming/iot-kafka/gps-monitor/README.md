# `gps-monitor` — build & run

Provided Java for the [iot-kafka-lab](../../iot-kafka-lab.md) lab — the Streams app that detects frequent parking.
...
docker run --network docker_kafka_network --rm -it -v "$PWD:/pwd" -w /pwd openjdk:11 \
  java -jar target/gps-monitor-1.0.0-SNAPSHOT.jar
```
