# `slow-consumer` — build & run

Provided Java for the [slow-poll-eviction](../slow-poll-eviction.md) lab — the deliberately slow consumer (max.poll.interval.ms eviction demo).
...
docker run --network docker_kafka_network --rm -it -v "$PWD:/pwd" -w /pwd openjdk:11 \
  java -DprocessMs=1000 -DmaxPollRecords=10 -DmaxPollIntervalMs=5000 -Dtopic=poll-test -jar target/slow-consumer-1.0.0-SNAPSHOT.jar
```
